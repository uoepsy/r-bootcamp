...
As we said, some package are pre-shipped with your R installation. Examples are the base, stats, and graphics packages. Each of those can be considered as a "folder" providing different functionality. You will see the use of those functions later on in the course.

<center>
![](images/packages-example.png){width=60%}
</center>

Actually, when you install R it comes with many more packages. Some are automatically loaded when you open R, while others are left in the background unopened. The functionality of the packages that are automatically loaded will be available to you immediately.
However, to use any functionality from packages that are not loaded, you need to first load the package using the `library()` function. 
...
To install packages that do not come with the basic R installation, you need to use the `install.libraries()` function or the "Packages" tab in the Files and Plots panel.

![](images/install-load-package.png)
