...
R can store information by creating an _object_ with a name of our choice. Typically, it is recommended to use a name that makes sense for the value that is being stored.
...
Here, the error message tells us that the object `X` does not exist. We can see the list of existing objects in the Environment tab in the upper-right window, where we will find that the correct object is `x`. It is also possible to obtain the same list of created objects by using the `ls()` function.
...
There are many other classes in R, and these will be introduced along this course.
...
As you saw above, the `sum()` function takes a vector and sums up all values inside.
...
Note that none of these operations change the original vector, because we have not ovewritten the object via an assignment.
...
Functions are pieces of code that take one or more inputs (also called arguments), do some computation on those, and return an output.

In the code `sqrt(25)`, `sqrt` is the function and `25` is the input or argument.

We have already seen some functions, for example `sqrt()`, `mean()`, `class()`, and `c()`.
...
- [Chapter 2, Stat 545](https://stat545.com/r-basics.html)

I know, the website above prefers using `<-` for assignment, rather than `=`. 

> Don’t be lazy and use =, although it would work, because it will just sow confusion later. Instead, utilize RStudio’s keyboard shortcut: Alt + - (the minus sign).

However, as you will see during your academic career, there is no consensus on many topics, and I personally prefer the `=` sign. You will see the different people use either for assignment.
