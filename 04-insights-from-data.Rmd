...
ggplot(.) +
    ...
    labs(x = 'Sample mean') +
    geom_function(fun = ~ dt(., df = nrow(btdata) - 1), color = 'red', size = 1) +
    annotate(geom = 'label', x = 3, y = 0.1, label = 't(49) distribution',
             color = 'red', size = 5) + 
    theme_void()
```

As you can see above, the distribution of the sample mean follows a t distribution (the red curve above). You don’t need to learn the mathematical formula of the red curve, as R can compute it for you (more later on).

The width of the distribution is the standard deviation of the sample mean. It has a special name however, __standard error__, as it tells us how variable the sample means are from sample to sample. In other words, it tells us how reliable those means are as estimates of the unknown population mean. The smaller the variation (the narrows the distribution), the better.

$$
SE(sample\ mean) = \frac{SD(original\ sample)}{\sqrt{sample\ size}}
$$
...
$$
SE(mean) = \frac{`r round(btstats$SD, 7)`}{\sqrt{50}} = 0.0601292
$$

You can confirm that it is approximately equal to the standard deviation of the 1000 generated sample means. They will become closer
and closer as the number of resamples increases (from 1000 to 10,000 or 100,000 for example):
...
Hence, the sample mean is `r round(btstats$M, 2)` °C, with a standard error of 0.06 °C.
...
Recall the distribution of the sample means:
...
The answer lies in comparing the distance between the sample mean and the hypothesised population value to the variability of the sample means due to random sampling.  
This is called the _t-statistic_ and is computed using:

<aside>
t-statistic
</aside>

- sample mean: the mean of the original sample.

- hypothesised value: the hypothesised value for the population parameter in the null hypothesis $H_0$.

- SE(sample mean): the standard error.
This is the SD of the sampling distribution of the mean.  __Careful:__ NOT the SD of the original sample!

The formula of the t-statistic is:

$$
\begin{aligned}
t 
&= \frac{\text{pattern}}{\text{noise}} \\
&= \frac{\text{distance of sample to hypothesis}}{\text{noise}} \\
&= \frac{\text{sample mean} - \text{hypothesised value}}{SE(\text{sample mean})}
\end{aligned}
$$

We typically decide that there is something going on in the data if the t-statistic is very far from 0; i.e. very large or very negative.  In those cases, __the pattern is larger than the noise due to random sampling alone__, and we say there is evidence against the null hypothesis and in favour of the alternative hypothesis.

If the t-statistic is not big enough, i.e. it is closer to 0, then we say that there is not sufficient evidence against the null hypothesis and hence we do not reject it. That is, we maintain / fail to reject the status quo.
...
- The original sample follows a normal distribution. This is a distribution similar to the t, and you do this test with the `shairo.test()` function in R:

Or:

- The sample size should be greater than 30 and no strong outliers present in the data.
...
If we checked the second, we would also have seen that it holds, but it is sufficient to check one of the two.

<details>
<summary>The other check</summary>
```{r}
dim(btdata)
```
...
<!-- qplot(x = S, geom = 'histogram', color = I('white')) + -->
<!--     labs(x = 'Sample mean') -->
<!-- ``` -->

<!-- ```{r} -->
<!-- hist(S, col = 'lightblue1', border = 'lightblue3',  -->
<!--      main = '', xlab = 'Sample mean') -->
<!-- plot(density(S)) -->
<!-- abline(v = quantile(S, c(0.025, 0.975)), col = 'red', lwd = 2) -->
<!-- ``` -->

<!-- ```{r} -->
<!-- ggplot(sldep, aes(x = Treatment, y = PDI)) +  -->
<!--     geom_boxplot() + -->
